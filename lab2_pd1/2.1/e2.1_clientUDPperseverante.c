...
int main (int argc, char *argv[]) {
	...
	fd_set cset;
	timeout.tv_sec = SECONDS; //imposto il # di secondi da attendere per timeout
	...
	int i=0;
	int retransmission_counter = 0;	//il client deve poter ritrasmettere la richiesta per 5 volte prima di terminare
	int transmitted = FALSE; //booleano per dire che se ho trasmesso correttamente allora posso uscire dal while della ritrasmissione
   ...
   while(retransmission_counter != MAXCOUNT){
		...
		* **/
		//azzero i campi che fanno sì che la select si blocchi: timeout e cset
		timeout.tv_sec = SECONDS; //imposto il # di secondi da attendere per timeout
		timeout.tv_usec = 0;
		FD_CLR(s, &cset);
		FD_ZERO(&cset);
		...
		if(result < strlen(msg)){
			...
			if(n > 0){	//la select è stata svegliata dal server, che ha inviato la risposta
				...
				if(n != -1){
					printf("answer: %s\n", answer);
					transmitted = TRUE;
					break;	//ho ricevuto correttamente la risposta, quindi esco dal ciclo while
				}else{
	...
	if(transmitted == TRUE){
		printf("The server answered with: %s\n", msg);
	}else{
		printf("No answer from the server after %d attempts of transmission.\n", MAXCOUNT);
	}
