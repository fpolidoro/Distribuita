...
int main (int argc, char *argv[]) {
	...
	if ( inet_aton(argv[1], &saddr.sin_addr) == 0 )
   {
	...
	printf("\n................................................................................\n");
	printf("COMMANDS:\n\tGET <filename> - sends a request for the file called filename\n\tQ - sends the quit message to the server\n\tA - immediately close the connection (stops any pending transfer)\n");
	printf("................................................................................\n\n");
	
	/**	NOTA: funziona bene, ma l'output si mischia un po' tra § e >, sarebbe meglio sostituire tutto
	 * 
	 * */
	...
	while(!quit){
		...
		if(result > 0){
			if(FD_ISSET(s, &cset)){	//sto ricevendo
				...
				fd = open(filename_copy, O_RDWR | O_CREAT | O_TRUNC, S_IRWXU | S_IRWXG | S_IRWXO);
				if (fd == -1) {
			...
			if(FD_ISSET(STDIN_FILENO, &cset)){	//l'utente ha scritto qualcosa su stdin
			/**
			 ...
			 * */
				if(fgets(string, STRLEN-2, stdin) == NULL){	//se scrivo e poi faccio CTRL+D, fgets non ritorna NULL. Lo ritorna solo se faccio CTRL+D senza scrivere nulla
					//EOF via CTRL+D
					printf("CTRL+D was pressed. Ignoring\n");
				}else{
					if(strncasecmp(string, GETMSG, strlen(GETMSG)) == 0){	//il comando è una GET
						printf("§ sending GET message to %s\n", argv[1]);
						nptr = strchr(string, '\n');
						if(nptr != NULL){
							*nptr = '\r';	//al char puntato da nptr assegno \0, cioè a \n sostituisco \0
							*(nptr+1) = '\n';
						}
						
					/** NOTA: Quando invio messaggi di lunghezza non fissa e il server legge fino a \n, devo usare
					 * 	strlen(string) invece che sizeof, altrimenti nel secondo caso, spedirei anche tutti gli \0
					 *  non usati, e al ciclo successivo di read del server, leggerei questi \0-schifezza di string
					 * */
						if((result = sendn(s, string, strlen(string), 0)) <= 0){
							if(result == 0)
							...
							nptr = strtok(string, " ");
							nptr = strtok(NULL, "\r");
							filename = strdup(nptr);
							printf("§ GET sent. Preparing for receiving file %s\n", filename);
						}
					}else if(strncasecmp(string, AMSG, strlen(AMSG)) == 0){	//comando = A, interrompe subito qualsiasi trasferimento + connessione
						printf("Stopping the communication immediately.\n");
						return 1;
					}else if(strncasecmp(string, QMSG, strlen(QMSG)) == 0){	//comando = Q, interrompe la comunicazione inviando QUIT al server
